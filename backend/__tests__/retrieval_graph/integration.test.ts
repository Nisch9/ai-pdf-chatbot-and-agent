...
/**
 * Deterministic embeddings for testing (no API key needed)
 */
class DeterministicEmbeddings extends Embeddings {
  ...
  private generateVector(text: string): number[] {
    ...
    for (let i = 0; i < this.dimensions; i++) {
      const seed = baseHash + i * 73856093;
...
describe('Retrieval Graph Integration', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      // Setup test documents in Supabase using deterministic embeddings
      ...
      console.error('3. Vector column is set to vector(384)\n');
  ...
  describe('Direct Answer Path', () => {
    it('should provide direct answers for general knowledge questions', async () => {
      ...
      // Should have human message and AI response
      expect(result.messages).toHaveLength(2);
      expect(result.messages[0]).toBeInstanceOf(HumanMessage);
      // Check second message has content (AI response)
      expect((result.messages[1] as any).content).toBeDefined();
  ...
  describe('Retrieval Path', () => {
    ...
    it('should retrieve and provide security audit details', async () => {
      ...
      // Model should either provide the answer or say it doesn't know
    ...
    it('should handle questions without relevant context gracefully', async () => {
      ...
      expect(result.messages).toHaveLength(2);
      const content = String(result.messages[1].content);
      // Just check that a response was provided
      expect(content).toBeTruthy();
      expect(content.length).toBeGreaterThan(0);
    }, 60000);
  });
